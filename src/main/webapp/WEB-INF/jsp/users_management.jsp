...
<t:layoutAfterLogin>
    <div class="container">
        <button type="button" class="btn item background" data-toggle="modal" data-target="#newUserModal">Dodaj użytkownika</button>
    ...
    <div>
        ...
        <c:forEach var="user" items="${users}" varStatus="counter">
            ...
            <td class="item"><form method="post" action="<c:out value="${delete_user}"/>"><input type="hidden" name="id" value="<c:out value="${user.id_uzytkownika}"/>"/><input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/><input type='submit' class="btn" value='Usuń użytkownika'></form><br></td>
            </tr>
        </c:forEach>
        </table>
    </div>
